<?php
/**
 * Formulario de Galería (crear/editar) — Panel de Administración
 */
use App\Core\View;
...
$submitUrl = $isEditing
    ? ADMIN_URL . '/gallery/update/' . (int) $editing->getAttribute('id')
    : ADMIN_URL . '/gallery/store';
$values = $isEditing ? $editing->toArray() : [
    'title' => '', 'description' => '', 'image' => '', 'is_active' => 1, 'sort_order' => 0,
];
?>
<div class="max-w-2xl">
    <div class="flex items-center gap-4 mb-8">
        <a href="<?= ADMIN_URL ?>/gallery" class="w-10 h-10 rounded-xl bg-darksoft border border-white/10 flex items-center justify-center text-cream/70 hover:text-goldlight hover:border-gold/40 transition">
            <i class="fa-solid fa-arrow-left"></i>
        </a>
        <div>
            <h2 class="font-display text-2xl font-semibold text-white"><?= View::e($title) ?></h2>
            <p class="mt-1 text-sm text-cream/50">Configura la foto, su título y posición en la galería.</p>
        </div>
    </div>
    ...
    <form method="POST" action="<?= $submitUrl ?>" enctype="multipart/form-data" class="bg-darksoft rounded-2xl border border-white/5 p-8 space-y-6">
        ...
        <div class="flex items-center gap-5">
            <div id="image-preview" class="w-24 h-20 rounded-2xl overflow-hidden bg-dark border border-white/10 flex items-center justify-center flex-shrink-0">
                ...
                <?php else: ?>
                    <i id="preview-icon" class="fa-solid fa-image text-gold/50 text-2xl"></i>
                <?php endif; ?>
            </div>
            <div class="flex-1">
        ...
        <div>
            <label class="block text-[11px] uppercase tracking-[.2em] text-cream/60 mb-2" for="title">Título *</label>
            <input type="text" id="title" name="title" required maxlength="100" value="<?= View::e($values['title'] ?? '') ?>" placeholder="Ej: Corte degradado + barba" class="w-full px-4 py-3 rounded-xl bg-dark/60 border border-white/10 text-white text-sm outline-none focus:border-gold/60 focus:ring-2 focus:ring-gold/20 placeholder:text-cream/30">
        </div>
        ...
        <div>
            ...
            <textarea id="description" name="description" rows="3" placeholder="Breve descripción del corte (se muestra al ampliar la foto)" class="w-full px-4 py-3 rounded-xl bg-dark/60 border border-white/10 text-white text-sm outline-none focus:border-gold/60 focus:ring-2 focus:ring-gold/20 placeholder:text-cream/30"><?= View::e($values['description'] ?? '') ?></textarea>
        </div>
        ...
        <div>
            ...
            <input type="number" id="sort_order" name="sort_order" min="0" value="<?= (int) ($values['sort_order'] ?? 0) ?>" class="w-full px-4 py-3 rounded-xl bg-dark/60 border border-white/10 text-white text-sm outline-none focus:border-gold/60 focus:ring-2 focus:ring-gold/20">
            <p class="mt-1 text-[11px] text-cream/40">Los números más bajos aparecen primero en el sitio web.</p>
        </div>
        ...
        <div class="flex items-center gap-3 pt-2">
            ...
            <label for="is_active" class="text-sm text-cream/80 cursor-pointer">Foto activa (visible en el sitio web)</label>
        </div>
        ...
        <div class="pt-4 flex flex-col sm:flex-row gap-3">
            <button type="submit" class="flex-1 inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl bg-gold text-darkdeep font-bold uppercase text-xs tracking-[.2em] hover:bg-goldlight transition shadow-lg shadow-gold/20 btn-shine">
                ...
                <?= $isEditing ? 'Guardar cambios' : 'Agregar foto' ?>
            </button>
            <a href="<?= ADMIN_URL ?>/gallery" class="inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl border border-white/10 text-cream/70 font-semibold text-xs uppercase tracking-widest hover:border-gold/40 hover:text-goldlight transition">
                Cancelar
            </a>
        </div>
    </form>
