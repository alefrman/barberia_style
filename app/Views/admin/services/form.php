<?php
/**
 * Formulario de Servicio (crear/editar) — Panel de Administración
 */
use App\Core\View;
...
$submitUrl = $isEditing
    ? ADMIN_URL . '/services/update/' . (int) $editing->getAttribute('id')
    : ADMIN_URL . '/services/store';
$values = $isEditing ? $editing->toArray() : [
    'name' => '', 'description' => '', 'price' => '', 'duration' => 30,
    'category_id' => 0, 'image' => '', 'is_active' => 1, 'sort_order' => 0,
];
?>
<div class="max-w-2xl">
    <div class="flex items-center gap-4 mb-8">
        <a href="<?= ADMIN_URL ?>/services" class="w-10 h-10 rounded-xl bg-darksoft border border-white/10 flex items-center justify-center text-cream/70 hover:text-goldlight hover:border-gold/40 transition">
            <i class="fa-solid fa-arrow-left"></i>
        </a>
        <div>
            <h2 class="font-display text-2xl font-semibold text-white"><?= View::e($title) ?></h2>
            <p class="mt-1 text-sm text-cream/50">Configura el servicio, su categoría y precio.</p>
        </div>
    </div>
    ...
    <form method="POST" action="<?= $submitUrl ?>" enctype="multipart/form-data" class="bg-darksoft rounded-2xl border border-white/5 p-8 space-y-6">
        ...
        <div class="flex items-center gap-5">
            <div id="image-preview" class="w-24 h-24 rounded-2xl overflow-hidden bg-dark border border-white/10 flex items-center justify-center flex-shrink-0">
                ...
                <?php else: ?>
                    <i id="preview-icon" class="fa-solid fa-scissors text-gold/50 text-2xl"></i>
                <?php endif; ?>
            </div>
            <div class="flex-1">
        ...
        <div>
            ...
            <input type="text" id="name" name="name" required value="<?= View::e($values['name'] ?? '') ?>" placeholder="Ej: Corte clásico" class="w-full px-4 py-3 rounded-xl bg-dark/60 border border-white/10 text-white text-sm outline-none focus:border-gold/60 focus:ring-2 focus:ring-gold/20 placeholder:text-cream/30">
        </div>
        ...
        <div>
            ...
            <textarea id="description" name="description" rows="3" placeholder="Breve descripción del servicio" class="w-full px-4 py-3 rounded-xl bg-dark/60 border border-white/10 text-white text-sm outline-none focus:border-gold/60 focus:ring-2 focus:ring-gold/20 placeholder:text-cream/30"><?= View::e($values['description'] ?? '') ?></textarea>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
            ...
            <div>
                <label class="block text-[11px] uppercase tracking-[.2em] text-cream/60 mb-2" for="duration">Duración (min) <span class="text-cream/40 normal-case">(opcional)</span></label>
                <input type="number" id="duration" name="duration" min="0" max="600" value="<?= (int) ($values['duration'] ?? 30) ?>" class="w-full px-4 py-3 rounded-xl bg-dark/60 border border-white/10 text-white text-sm outline-none focus:border-gold/60 focus:ring-2 focus:ring-gold/20">
            </div>
            <div>
        ...
        <div>
            ...
            <div id="new-category-wrap" class="hidden mt-3">
                ...
                <div class="flex gap-2">
                    <input type="text" id="new_category" name="new_category" placeholder="Ej: Cortes de cabello" class="flex-1 px-4 py-3 rounded-xl bg-dark/60 border border-white/10 text-white text-sm outline-none focus:border-gold/60 focus:ring-2 focus:ring-gold/20 placeholder:text-cream/30">
        ...
        <div class="pt-4 flex flex-col sm:flex-row gap-3">
            ...
            <a href="<?= ADMIN_URL ?>/services" class="inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl border border-white/10 text-cream/70 font-semibold text-xs uppercase tracking-widest hover:border-gold/40 hover:text-goldlight transition">
                Cancelar
            </a>
        </div>
    </form>
